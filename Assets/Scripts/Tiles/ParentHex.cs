...
public class ParentHex : TileParent
{
    ...
    [SerializeField] Transform parColliders, parDisplays;
    [SerializeField] Transform parSpritesRed;
    [SerializeField] Transform parSpritesBlue;
    [SerializeField] GameObject trajectoryCollider;
    Collider2D[] _colliders;
    GameObject[] _displayValues;
    GameObject[] _spritesRed, _spritesBlue;
    const int CONST_HexNum = 10;
    byte _mySiblingsIndex;

    [Title("Final Hex")]
    [SerializeField] Transform finalHexagonTransform;
    [SerializeField] ParticleSystem[] psTaken;
    MeshRenderer _finalHexMesh;
    [SerializeField] TextMeshPro finalHexScore;
    [SerializeField] MeshRenderer finalHexFrameMesh;
    [SerializeField] SpriteRenderer finalBackgroundSpriteRend;
    [SerializeField] Sprite[] finalBackgroundSprites;
        ...
        private set
        {
            ...
            switch (_tState)
            {
                ...
                case TileState.Taken:
                    ...
                    psTaken[CurrentValue > 0 ? 0 : 1].Play();
                    break;
            }
        }
    }
    ...
    int NeighboursCheckIn
    {
        ...
        set
        {
            ...
            AudioManager.Instance.PlaySfx(AudioManager.Instance.hexCounter, true, finalHexagonTransform.position);
            if (value <= 0) //all neighbours have finished their animations
            {
                if (_oneShotNeighbourCheckIn) return;
                _oneShotNeighbourCheckIn = true;
                gm.NextPlayer_ServerRpc(gm.playerTurnNet.Value, false, "NeighboursCheckIn");
            }
        }
    }
    ...
    [ReadOnly] public int valueForBot;
    ...
    private void Awake()
    {
        ...
        _spritesRed = Utils.AllChildrenGameObjects(parSpritesRed);
        _spritesBlue = Utils.AllChildrenGameObjects(parSpritesBlue);
    ...
    void InactiveHex()
    {
        ...
        Utils.Activation(parColliders.gameObject, GenActivation.Off);
        Utils.Activation(parDisplays.gameObject, GenActivation.Off);
        Utils.Activation(parSpritesRed.gameObject, GenActivation.Off);
        Utils.Activation(parSpritesBlue.gameObject, GenActivation.Off);
        Utils.Activation(finalHexagonTransform.gameObject, GenActivation.Off);
        Utils.Activation(trajectoryCollider, GenActivation.Off);
    }
    public void HexHit(sbyte ordinal)
    {
        if (Tstate == TileState.InActive || Tstate == TileState.Taken) return;

        int mod = gm.playerTurnNet.Value == PlayerFaction.First_left ? 1 : -1;
        ...
        {
            gm.NextPlayer_ServerRpc(gm.playerTurnNet.Value,false, "HexHit");
            return;
        }
            ...
            case TileState.Free:
                switch (valueByte)
                {
                    case > 0:
                        ...
                        _finalHexMesh.material = gm.factionData[0].matFinalHex;
                        Utils.ActivateOneArrayElement(_spritesRed, positiveValue);
                        break;
                    case < 0:
                        Utils.ActivateOneArrayElement(_displayValues, positiveValue);
                        _finalHexMesh.material = gm.factionData[1].matFinalHex;
                        Utils.ActivateOneArrayElement(_spritesBlue, positiveValue);
                        break;
                    default:
                        spriteRenderer.enabled = true;
                        Utils.ActivateOneArrayElement(_spritesBlue);
                        Utils.ActivateOneArrayElement(_spritesRed);
            ...
            case TileState.Taken:
                if (positiveValue != 0)
                {
                   ...
                   // finalHexScore.color = gm.playerDatas[valueByte > 0 ? 0 : 1].colMain;
                    ...
                    finalHexFrameMesh.material = gm.factionData[valueByte > 0 ? 0 : 1].matMain;
                    finalHexFrameMesh.enabled = true;
                    finalHexagonTransform.localEulerAngles = new Vector3(60f, 90f, -90f);
                    finalBackgroundSpriteRend.sprite = finalBackgroundSprites[valueByte > 0 ? 0 : 1];
                    finalBackgroundSpriteRend.enabled = true;
                    finalBackgroundSpriteRend.DOFade(1f, 1f);
                }
                if (valueByte > 0) _finalHexMesh.material = gm.factionData[0].matFinalHex;
                else _finalHexMesh.material = gm.factionData[1].matFinalHex;
                break;
        }
