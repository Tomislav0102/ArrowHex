...
public class GridManager
{
    ...
    [SerializeField] TextMeshPro levelNameText;
    ...
    public void ChooseGrid(GameObject levelToCreate = null)
    {
        if (gm.gridValuesNet.Count == 0)
        {
            ...
            else
            {
               ...
               //temp
               int levelIndex = Random.Range(0, allLevels.Length);
               levelNameText.text = $"Level {allLevels[levelIndex].name}";
               ChooseGrid(allLevels[levelIndex]); 
            }
        }
        else
        {
    ...
    void GridRandom()
    {
        _size = (GenSize)PlayerPrefs.GetInt(Launch.Instance.persistentDataManager.gameData.size_Int);
        int sizeBorder = 0;
        switch (_size)
        {
            case GenSize.Small:
        ...
        levelNameText.text = "Procedural level";
    }
    public void GridUseNetworkVariables()
    {
        ...
        levelNameText.text = "Host knows name of this level...";
    }
