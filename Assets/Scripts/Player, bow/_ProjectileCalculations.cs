...
public class DrawTrajectory
{
    ...
    public void Trajectory(Transform spawnPoint, PlayerFaction playerActive, float projectileMass, float strength, bool draw = true)
    {
        if (!draw || !showTrajectory)
        {
            trajectoryLr.enabled = false;
    ...
    void DrawProjection(Transform spawnPoint, float projectileMass, float throwStrength)
    {
        ...
        for (float time = 0; time < CONST_LinePoints; time += CONST_TimeBetweenPoints)
        {
            ...
            if (Physics.Raycast(lastPosition, (point - lastPosition).normalized, out RaycastHit hit, (point - lastPosition).magnitude, Launch.Instance.persistentDataManager.gameData.layForTrajectory))
            {
