...
/// Has a 'ISerializationCallbackReceiver' because 2D arrays cant serialize
/// Can generate, randomize and save existing grid. Grid is saved as a prefab.
/// </summary>
public class LayoutGenerator : MonoBehaviour, ISerializationCallbackReceiver
{
    ...
    void GenerateGrid()
    {
        //pomak po x - vel * (0.5f * Mathf.Sqrt(3))
        //pomak po y - vel * 0.75f
        DestroyAll();
        markersInEditor = new Marker[dim.x, dim.y];
        offsetPosition += 1f;
